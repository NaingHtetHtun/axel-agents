...
export async function loadRegistry() {
	...
	for (const line of content.split("\n")) {
		...
		if (packageMatch) {
			...
			packages[current] = { name: current, depends: [] };
			continue;
		}
		...
		if (dependencyMatch && current)
			packages[current].depends.push(dependencyMatch[1]);
	}
	if (!packages.core?.source)
...
export async function loadAdapter(name) {
	...
	const content = await readFile(
		path.join(root, "packages", "adapters", name, "adapter.yaml"),
		"utf8",
	);
