...
Usage:
  agents init [--stack laravel,react,nestjs] [--adapter opencode] [--force]
  agents add <extension>
  agents update
  agents doctor
  agents onboard
  agents context
...
try {
  switch (command) {
    case 'init':
      ...
      break;
    case 'add':
      if (!args[0]) throw new Error('Usage: agents add <extension>');
...
async function selectedStacks(args) {
  ...
  const answer = await prompt.question('Choose stacks (comma-separated, blank for core only): ');
  prompt.close();
