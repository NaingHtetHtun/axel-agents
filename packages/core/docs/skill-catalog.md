# Skill Catalog

| Category | Foundation skills |
| --- | --- |
| architecture | clean-architecture, dependency-injection, api-design |
| backend | service-boundaries |
| database | data-modeling |
| security | security-baseline, input-validation |
| performance | performance-baseline |
| testing | testing-strategy |
| debugging | debugging-method |
| git | git-conventions |
| documentation | documentation-standard |
| deployment | release-safety |
| quality | clean-code, solid, dry, kiss-yagni |
| planning | feature-planning |
| review | code-review |
| shared | naming, error-handling, logging, observability |
| backend / laravel | controllers, service-layer, repositories, form-request, api-resource, eloquent, migrations, queues, events, policies, sanctum, caching, scheduler, testing, deployment |
| frontend / react | components, hooks, state-management, routing, testing, performance |
| backend / go | project-structure, http-services, concurrency, data-access, testing |
| backend / nestjs | controllers, modules, services, middleware, guards, pipes, interceptors, dependency-injection, testing, deployment |

The remaining category directories are deliberately reserved for future extensions such as Python. Add a skill only when it owns a distinct decision or workflow; do not create aliases that duplicate an existing responsibility.
